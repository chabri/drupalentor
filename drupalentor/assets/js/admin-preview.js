...
(function ($, Drupal, drupalSettings) {
    ...
    $('#drupalentor-preview').on('load', function() {
        // Inyectar la función en el iframe
        this.contentWindow.drupalentorOpenModal = drupalentorOpenModal;
        this.contentWindow.drupalentorAddColum = drupalentorAddColum;
    ...
    $(document).keydown(function (e) {
        // Verificar si se presionó Control (Ctrl o Command en Mac) y la tecla 'S'
        if ((e.ctrlKey || e.metaKey) && e.keyCode === 83) {
            // Evitar que el navegador maneje la combinación de teclas (por defecto, abrirá la ventana de guardar)
            e.preventDefault();
    
            // Realizar el submit del formulario
            $('#drupalentor_form_builder').submit();
        }
    });
    
    $(document).ajaxStart(function() {
        $('body').append('<div class="drupalentor_loading"><div class="spinner-grow text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>');
    });
    
    $(document).ajaxStop(function() {
        $('.drupalentor_loading').remove();
    });
    ...
    $(document).on("keyup", '[data-update-selector]', function(e){
        ...
        var contenidoIframe = $('#drupalentor-preview').contents().find(selector);
        $(contenidoIframe).text($(this).val());
    })

    $(document).on("change", '[data-update-selectorhtml]', function(e){
        e.preventDefault();
        var contenidoIframe = $('#drupalentor-preview').contents();
    ...
    $(document).ready(function() {
        ...
        var contenidoIframe = $('#drupalentor-preview').contents().find('#drupalentor-builder').html();

        $('.drupalentor-preview-content').resizable({
            handles: {
              'e': '.r',
              'w': '.l'
            }
        ...
        $("#btn-desktop").on("click", function() {
            $(".drupalentor-preview-content").css({
                "width": "100%",
            });
        });
        ...
        $('#drupalentor_form_builder').on("submit", function(e){
            e.preventDefault();
            const formData = getWidgetStructure();
  
            drupalentorSavePage(formData);
    ...
    function addClasses(classes){
        ...
        var iframeContent = $('#drupalentor-preview').contents();
    ...
    function getWidgetStructure(){
        ...
        $(contenidoIframe).find('.widget-cloned').each(function() {
            ...
            drupalentorSaveWidgetfromStruture(dataSettings);
    ...
    function drupalentorOpenModal(form){
        var modal = '<div class="drupalentor-modal">';
        modal += '<div class="drupalentor-modal_container">';
        modal += form;
        modal += '</div>';
        modal += '</div>';
        $('body').append(modal);
        ...
        const formElement = $('.drupalentor-modal form');
        const editorElement = document.querySelector('[drupalentor-editor]');
        ...
        .then(editor => {
            editor.model.document.on('change:data', () => {
                ...
                var contenidoIframe = $('#drupalentor-preview').contents().find('#widget-id-' + settings.wid + ' ' + selector);
        ...
        $('#add_multiple_images_field').on('click', function (e) {
            ...
            var element_id = $(this).data('element-id');
            var elements =  $('.drupalentor_gallery_field').find('.image-box').length;
        ...
        formElement.on("submit", function(e){
            ...
            drupalentorSave(formData);
            $('#drupalentor-preview').contents().find('#widget-id-' + settings.wid).attr('data-settings', JSON.stringify(formData));
            // document.getElementById('drupalentor-preview').contentWindow.location.reload(true);
            return false;
        })

        $('.drupalentor_textarea').on('change', function (param) {
            var selector = $(this).data('update-selector');
        ...
        $('.drupalentor-edit-grid-item').on("click", function(e){
            e.preventDefault();
        })
        $('.drupalentor-remove-grid-item').on("click", function(e){
            e.preventDefault();
            $(this).closest('.image-box').remove();
        })


        $('.drupalentor-add-item').on("click", function(e){
            e.preventDefault();
            
            var htmlData = $(this).data('html');
           
            
            var count = $(this).closest('.drupalentor_multiple_field').find('.accordion-item').length;
            ...
            $(this).closest('.drupalentor_multiple_field').find('.accordion').append(updatedHtml);
          

        })
        $('.drupalentor-close-modal').on("click", function(e){
            e.preventDefault();
            $('.drupalentor-modal').remove();
        })
        $(".drupalentor_gallery_field .gallery-images-wrapper").sortable({
            items: '.image-box',
            handle: ".drupalentor-move-grid-item",
            cursor: "move",
            opacity: 0.5,
            ...
            stop: function(event, ui) {
                ...
                $(".gallery-images-wrapper .image-box").each(function(index) {
                    ...
                    $(this).find('.drupalentor-hover-modal').attr('id', hoverModalId);

                    // Actualiza el atributo 'data-show-click' en el botón de editar.
                    $(this).find('.drupalentor-edit-grid-item').attr('data-show-click', '#' + hoverModalId);

                    // Actualiza los atributos 'name' de los inputs dentro del hover-modal.
                    $(this).find('.drupalentor-hover-modal input[name^="element[gallery_items]"]').each(function() {
    ...
    function openMediaModal(element_id, type, total){
        ...
        $.ajax({
        url: '/admin/drupalentor/media-modal', // Reemplaza con la URL de tu ruta de Ajax.
        method: 'POST',
        ...
        success:function(data) {
            ...
            if($(modal).hasClass('modal-type-multiple')){
                ...
                $('.insert-media-modal').on('click', function() {
                    ...
                    var index = (total + i);
                      selectedHtml += '<div class="col-xs-3 image-box mb-2">';
                      selectedHtml += '<div class="drupalentor-edit-grid-item btn btn-sm btn-info position-absolute top-50 start-50 translate-middle rounded-circle" data-show-click="#edit-gallery-image-'+index+'"><i class="fa-solid fa-pen-to-square"></i></div>';
                      selectedHtml += '<div class="drupalentor-move-grid-item btn btn-sm btn-info position-absolute top-0 start-0 rounded-circle"><i class="fa-solid fa-arrows-up-down-left-right"></i></div>';
                      selectedHtml += '<div class="drupalentor-remove-grid-item btn btn-sm btn-danger position-absolute bottom-0 end-0 rounded-circle"><i class="fa-solid fa-trash"></i></div>';
                      selectedHtml += '<img src="' + selectedData[i].thumbnail + '" data-fid="' + selectedData[i].fileid + '">';

                      selectedHtml += '<div id="edit-gallery-image-'+index+'" class="drupalentor-hover-modal bg-light p-3 position-absolute top-50 start-0 translate-middle-y shadow-lg hidden w-100">';
                ...
                $('.insert-media-modal').on("click", function(e){
                    ...
                    $('.drupalentor-media-modal').remove();
                })
            }
            ...
            $('#drupalentor_upload_image').on('change', function () {
                ...
                $.ajax({
                    url: '/admin/drupalentor/upload_file',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (data) {
                        ...
                        for (var i = 0; i < files.length; i++) {
                            ...
                            $('.drupalentor-modal_container > .row').prepend('<div class="col-xs-2 image-box mb-2 selected" data-fileid="' + fileData.file_id + '" data-thumbnail="' + fileData.file_url + '"><span><img src="' + fileData.file_url + '"></span></div>');
                        ...
                        // $('.insert-media-modal').attr('data-fileid', data[0].file_id); // Suponiendo que deseas utilizar solo el primer archivo como valor por defecto
                        $('.image-box').on('click', function (e) {
                            e.preventDefault();
                            $('.image-box').removeClass('selected');
                            $(this).addClass('selected');
    ...
    function addWidget(widget_id, did, langcode){
        ...
        var result="";
        $.ajax({
        url: '/admin/drupalentor/widget/' + widget_id + '/' + did + '/' + langcode, // Reemplaza con la URL de tu ruta de Ajax.
        method: 'POST',
        data: JSON.stringify(data),
        async: false,  
        success:function(data) {
    ...
    // Guardar Widget 1
    function drupalentorSaveWidgetfromStruture(settings) {
            ...
            success: function (data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var contenidoIframe = $('#drupalentor-preview').contents().find('head');
                        var stylesContainer = contenidoIframe.find('[data-drupalentor="' + drupalSettings.did + '"]');
                        if (stylesContainer.length) {
                            stylesContainer.text(data.styles);
                        } else {
                           
                            contenidoIframe.append('<style type="text/css" data-drupalentor="' + drupalSettings.did + '">' + data.styles + '</style>');
                        }
    ...
    // Guardar Widget
    function drupalentorSave(settings) {
        ...
        $.ajax({
            url: '/admin/drupalentor/save_widget',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function (data) {
    ...
    // Guardar Página
    function drupalentorSavePage(settings) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function (data) {
                        var stylesContainer =$('head').find('[data-drupalentor="' + drupalSettings.did + '"]');
                        if (stylesContainer.length) {
                          stylesContainer.text(data.styles);
                        } else {
                          $('head').append('<style type="text/css" data-drupalentor="' + drupalSettings.did + '">' + data.styles + '</style>');
                        }
                        createAlert('','Styles Saved or updated!','everything went well.','info',true,true,'pageMessages');     
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
    ...
    // Añadir columna
    function drupalentorAddColum(widget_id, did, langcode){
        $('#drupalentoAddColumn').modal('show');


        $('.drupalentor-column-modal .column-box').on('click', function(){
            var html = addWidget('drupalentor_column', did, langcode);

            $('#drupalentor-preview').contents().find('section[id="widget-id-'+widget_id+'"]').find('.row-elements').append(html);

            $('#drupalentor-preview').contents().find('div[id="'+$(html).attr('id')+'"]').attr('data-column-size', $(this).data('column')).addClass($(this).data('column'));
            $('#drupalentoAddColumn').modal('hide');
        });
    ...
    function addElementWidgetModal(element_id, did, langcode){
        ...
        $('.drupalentor-element-widget-modal .widget-box_element').off('click').on('click', function(){
            var element = $(this).data('element');
            var html = addWidget(element, did, langcode);
            $('#drupalentor-preview').contents().find('div[id="widget-id-'+element_id+'"]').find('.column-content-inner').append(html);
