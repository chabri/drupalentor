...
class ModalMediaController extends ControllerBase {
  ...
  public function mediaModal(Request $request) {
    ...
    $html = '<div class="drupalentor-media-modal modal-type-'. $data['type'] .'">';
    $html .= '<div class="drupalentor-modal_container">';
    $html .= '<div class="row mb-2">';
    foreach ($files as $file) {
      $file_uri = $file->getFileUri();
      $image_style_url = ImageStyle::load('thumbnail')->buildUrl($file_uri);

      $html .= '<div class="col-xs-2 image-box mb-2" data-fileid="'.$file->id().'" data-thumbnail="'.$image_style_url.'"><span><img src="' . $image_style_url . '" alt="Thumbnail"></span></div>';
    }
    $html .= '</div>';
    $html .= '<div class="upload-image">
              <form id="drupalentorUploadImageForm" enctype="multipart/form-data">
              <label>'.t('Upload Image').'</label>
              <input type="file" id="drupalentor_upload_image" name="drupalentor_upload_image" '.$data['type'].' title="Upload Image" accept="image/png, image/jpeg" class="form-control">
              </form>
              </div>';
