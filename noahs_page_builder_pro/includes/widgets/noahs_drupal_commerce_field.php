...
use Drupal\noahs_page_builder\WidgetBase;
   ...
   class element_noahs_drupal_commerce_field extends WidgetBase{
      ...
      public function render_form(){
         ...
         $product_types = \Drupal::entityTypeManager()->getStorage('commerce_product')->loadMultiple();
         $products = [];
         $products[''] = 'Select your product';
         foreach ($product_types as $item) {
           $products[$item->id() .','.  $item->bundle()] = $item->label();
         }
         ...
         $options = $this->productFields('commerce_product_type', 'commerce_product');
      ...
      public function template( $settings ){
         ...
         $element = '[commerce__product_field]';
     

         
         if(!empty($settings->drupal_field)){
            $element = '[commerce_product_'.$settings->drupal_field.']';
            if(!empty($settings->noahs_commerce_product)){
               ...
               $build = $product->get($settings->drupal_field)->view($view_mode);
               $element =  \Drupal::service('renderer')->render($build);
            }else{
               $node = \Drupal::routeMatch()->getParameter('commerce_product');

               if(!empty($node)){
                  $view_mode = 'noahs_view';
                  $view_builder = \Drupal::entityTypeManager()->getViewBuilder('commerce_product');
                  $build = $node->get($settings->drupal_field)->view($view_mode);
                  $element =  \Drupal::service('renderer')->render($build);
               }
            }
         }
         
         $class = !empty($settings->wrapp_element_class->text) ? $settings->wrapp_element_class->text : ''; 

         $output = '<div class="widget-content">';
         $output .= '<' . ($settings->wrapp_element ?? 'div') . ' class="' . $class . '">';
         $output .=  $element;
      ...
      private function getViews($pid, $bundle) {

      
         $viewModes = \Drupal::service('entity_display.repository')
         ->getViewModeOptionsByBundle($pid, $bundle);
