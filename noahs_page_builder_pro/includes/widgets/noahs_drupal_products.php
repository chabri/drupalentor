...
use Drupal\noahs_page_builder\WidgetBase;
   ...
   class element_noahs_drupal_products extends WidgetBase{
      ...
      public function render_form(){
         ...
         foreach ($media_types as $menu_entity) {
           $products[$menu_entity->id()] = $menu_entity->label();
         }
            ...
            $form['noahs_commerce_products'] = [
               ...
               'attributes' => [
                  'data-select-2' => true,
                  'data-select-2-sortable' => true,
               ],
               'group' => 'group_menu',
               'description' => 'The products are rendered as Teaser View Mode',
            ];
      ...
      public function template( $settings ){
         ...
         $output = '';
         $columns = !empty($settings->section_grid) ? $settings->section_grid : 'row-cols-1 row-cols-sm-2 row-cols-md-4';
         if(!empty($settings->noahs_commerce_products)){

            $output .= '<div class="w-100">';
            $output .= '<div class="row ' .$columns .'">';
            foreach($settings->noahs_commerce_products as $product_id){ 
               $entity = \Drupal::entityTypeManager()->getStorage('commerce_product')->load($product_id);
               $build = \Drupal::entityTypeManager()->getViewBuilder('commerce_product')->view($entity, 'teaser');
            ...
            $output = '<h4>Add your products</h4>';
         }
